      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="w-72 sm:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden"
          ...
          >
            ...
            {/* 入力エリア */}
            <div className="border-t border-gray-100 px-3 py-3 flex items-center gap-2 bg-white shrink-0">
              {/*
                min-w-0 が重要：flexアイテムのデフォルトmin-widthがautoのため、
                inputが縮まず送信ボタンがはみ出して隠れるバグを防ぐ
              */}
              <input
                type="text"
                value={input}
                ...
                disabled={isLoading}
                enterKeyHint="send"
                className="flex-1 min-w-0 text-sm px-3 py-2 bg-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-yellow-primary/50 disabled:opacity-50 transition"
              />
              {/*
                shrink-0 で送信ボタンが縮まないように固定する
                onPointerDown + preventDefault：iOSでタップ時にinputのblurより先に
                送信を確定させ、キーボードが閉じる前に処理が走るようにする
              */}
              <button
                onPointerDown={(e) => {
                ...
                className="w-10 h-10 bg-yellow-primary text-charcoal rounded-xl flex items-center justify-center shrink-0 hover:bg-yellow-dark hover:text-white transition-colors disabled:opacity-40"
                aria-label="送信"
              >
